...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="card-anime overflow-hidden rounded-3xl border border-white/10 bg-slate-900/90 p-2 shadow-xl md:backdrop-blur-md md:hover:scale-[1.02] md:transition-transform md:duration-150">
      ...
      <div className="space-y-3 px-2 pb-2 pt-4 text-center">
        ...
        {!isInCart ? (
          <button
            ...
            className="w-full rounded-2xl bg-gradient-to-r from-fuchsia-600 to-cyan-500 py-3 text-base font-black text-white md:hover:scale-[1.02] md:transition-transform md:hover:shadow-[0_0_20px_rgba(168,85,247,0.6)]"
          >
            أضف للسلة
          </button>
        ) : (
          <button
            onClick={() => removeItem(product.id)}
            className="w-full rounded-2xl border border-red-500/40 bg-red-500/10 py-3 text-base font-black text-red-300 md:hover:bg-red-500/20 md:transition-colors"
          >
            إزالة من السلة ❌
          </button>
        )}
